...
void func_80A4E22C(EnMkk* this, PlayState* play) {
    ...
    func_8002829C(play, &pos, &sZeroVec, &D_80A4F788, &D_80A4F780, &D_80A4F784, 350, 20);
}
...
void func_80A4E2E8(EnMkk* this, PlayState* play) {
    ...
    if (0) {//((player->stateFlags3 & 0x100) || (Player_GetMask(play) == PLAYER_MASK_STONE)) {
    ...
    this->actor.shape.rot.y =
        (s32)(Math_SinF(this->unk_14E * ((2 * M_PI) / 15)) * (614.4f * this->actor.speedXZ)) + this->unk_150;
    func_8002F974(&this->actor, NA_SE_EN_AMOS_DAMAGE - SFX_FLAG); 
    if (sp20) {
        this->unk_14B &= ~2;
        func_80A4E190(this);
    } else if ((this->unk_149 == 0) && (!(player->stateFlags3 & 0x100)) &&
               1 /*(Player_GetMask(play) != PLAYER_MASK_STONE)*/ && (this->actor.bgCheckFlags & 1) &&
...
void func_80A4E58C(EnMkk* this) {
    ...
    Audio_PlayActorSfx2(&this->actor, NA_SE_EN_ANUBIS_FIRE); 
...
void func_80A4E72C(EnMkk* this, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & 1) {
        if (this->actor.velocity.y > -1.0f) {
            ...
            EffectSsDeadDb_SpawnMM(play, &temp, &sZeroVec, &sZeroVec, &sEffPrimColors[this->actor.params],
    ...
    if (this->unk_14B & 3) {
        ...
        this->unk_154.x = this->actor.world.pos.x -
                          10.0f * Math_SinS(this->actor.shape.rot.y +
                                            (s32)(1228.8f * this->actor.speedXZ * Math_SinF(this->unk_14E * (M_PI / 5))));
...
void func_80A4EBBC(EnMkk* this, PlayState* play) {
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        Actor_SetDropFlag(&this->actor, &this->collider.info, false);
...
void func_80A4EE48(EnMkk* this, PlayState* play) {
    ...
    } else if ((this->actor.xzDistToPlayer < this->unk_178) && 1) {//(Player_GetMask(play) != PLAYER_MASK_STONE)) {
